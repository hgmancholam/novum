...
|---|---|---|
| **A** | Domain types + LLM role + prompts | `app/domain/meta_stop.py` (NEW), `app/domain/events.py`, `app/domain/enums.py`, `app/llm/roles.py`, `app/llm/prompts/meta_judge_voc.py` (NEW), `app/llm/prompts/meta_judge_adversarial.py` (NEW), `app/config.py` |
...
| **C** | Orchestrator hook STANDARD `after_judge` + fallback prompt | `app/agent/orchestrator.py`, `app/llm/prompts.py` (synth fallback clause), `app/agent/run_state.py` (meta-judge counters) |
...
- `app.llm.client.LLMProviderQuotaExhausted` (exception class, ya importada en orchestrator).
- `app.stopping.signals.no_progress.NoProgressSignal` con `priority=30`, contrato `StopSignalOutput`. Los nuevos signals siguen el mismo patrón.
...
| **T-26-A-04** | Añadir `LLMRole.META_JUDGE = "meta_judge"` al StrEnum en `roles.py`. Añadir entrada en `ROLE_CONFIGS` que reutilice el pool del JUDGE por defecto: `models=_resolve_pool(settings.llm_model_meta_judge_pool, settings.llm_model_meta_judge)`, `temperature=0.0`, `max_tokens=1024`, `description="Agentic stopping meta-judge (BRD-26)"`. | `backend/app/llm/roles.py` | S | [be-llm] |
| **T-26-A-05** | Añadir a `Settings` en `app/config.py` los campos: `llm_model_meta_judge: str = Field(default="<same default as judge>")` y `llm_model_meta_judge_pool: str = Field(default="")`. Documentar en docstring que un deploy puede sobrescribir vía `LLM_MODEL_META_JUDGE` / `LLM_MODEL_META_JUDGE_POOL`. Los env legacy del BRD (`META_JUDGE_PROVIDER`, `META_JUDGE_MODEL_<provider>`) NO se introducen — usamos el mismo esquema que el resto de roles (decisión D-XXX, registrar en memory bank). | `backend/app/config.py` | S | [be-config] |
| **T-26-A-06** | Crear `app/llm/prompts/__init__.py` vacío (módulo). Crear `app/llm/prompts/meta_judge_voc.py` con la constante `VOC_SYSTEM_PROMPT: str` (texto del BRD §4.9 *Value-of-Continuation*) y función `build_voc_user_prompt(state: RunState, judge_verdict: JudgeRuledEvent) -> str` que serializa: original question, AnswerKind, lane, sub-claims con evidence count + tier mix, S_effective actual, J_score, rondas ejecutadas, rondas restantes, último judge verdict (approve/reject + reason). Todo en inglés. | `backend/app/llm/prompts/meta_judge_voc.py` | M | [be-llm] |
| **T-26-A-07** | Crear `app/llm/prompts/meta_judge_adversarial.py` con `AC_SYSTEM_PROMPT: str` (texto del BRD §4.9 *Adversarial Completeness*) y `build_ac_user_prompt(state, draft_answer, evidence_index) -> str`. El prompt obliga **exactamente 3** objeciones diferenciadas (no variaciones), `suggested_query` ≤ 6 tokens. Todo en inglés. | `backend/app/llm/prompts/meta_judge_adversarial.py` | M | [be-llm] |
...
| `tests/test_llm_roles.py` | `test_meta_judge_role_defaults_to_judge_pool` | NEW |
| `tests/test_llm_roles.py` | `test_meta_judge_env_override` — `LLM_MODEL_META_JUDGE_POOL="claude-sonnet-4-6,deepseek-v3"` → tupla parseada | NEW |
| `tests/test_llm_prompts_meta_judge.py` | `test_voc_prompt_includes_required_state_fields` (S_effective, lane, rounds, last judge verdict) | NEW |
...
| **T-26-B-03** | En ambos signals: wrap `llm.call` con manejo de error específico. (a) `InstructorRetryException` (output malformado tras retry de instructor) → devolver fallback synthesized: `ValueOfContinuationVerdict(decision="stop_best_effort", expected_delta_s=0.0, next_action_hypothesis=None, reason="meta_judge_unparseable")`. (b) `LLMProviderQuotaExhausted` → fallback con `reason="meta_judge_quota_exhausted"`, **sin** retry (NFR-26-05). Loggar con structlog. | `backend/app/stopping/signals/meta_judge_voc.py`, `backend/app/stopping/signals/meta_judge_adversarial.py` | M | [be-stopping] |
| **T-26-B-04** | Registrar ambos signals en `app/stopping/signals/__init__.py` (`__all__` + import). Mantener orden de `priority` consistente con el resto de signals (Budget=20, NoProgress=30, Judge=40, VoC=45, AC=46). | `backend/app/stopping/signals/__init__.py` | XS | [be-stopping] |
...
| `tests/test_stopping_meta_judge_priority.py` | `test_signal_priorities_ordered` — Budget(20) < NoProgress(30) < Judge(40) < VoC(45) < AC(46) | NEW |
...
| `tests/test_objections_to_subclaims.py` | `test_route_objections_to_hypotheses_jaccard_overlap` | NEW |
...
Conectar VoC + AC en el orchestrator del carril STANDARD, justo después de `_handle_judging`, **antes** del check actual de `judge_attempts >= max_judge_attempts`. Añadir el clause de FALLBACK MODE al synthesizer cuando termina por `stop_best_effort`.
...
| **T-26-C-02** | Implementar `_after_judge_meta_decision` siguiendo BRD §4.6 al pie de la letra: (1) **happy path** — si `judge.verdict == APPROVE AND state.last_judge_confidence >= threshold` → `judge_confirmed`, no invocar meta-juez. (2) Invocar `ValueOfContinuationSignal.evaluate_with_state(state, hook="after_judge")`, persistir el `MetaStopVerdictEvent` vía `self.emit`. (3) Despachar por `verdict.decision`: `stop` → `judge_confirmed` si threshold cumplido sino `_draft_best_effort_fallback`; `stop_best_effort` → `_draft_best_effort_fallback`; `continue` con `expected_delta_s >= 0.03` → invocar `AdversarialCompletenessSignal`, persistir evento; si `all_answered` → `judge_confirmed`; sino → `objections_to_subclaims.convert`, append a `state.sub_claims`, emit `DirectedSubclaimsFromObjectionsEvent`, `state.judge_attempts += 1`, si `judge_attempts >= max_judge_attempts` → `_draft_best_effort_fallback`, sino → `transition_to(ANALYZING)`. | `backend/app/agent/orchestrator.py` | XL | [be-agent] |
...
| **T-26-C-04** | Cuando `_draft_best_effort_fallback` se invoca por `stop_best_effort` del meta-juez, pasar el `verdict.reason` al synthesizer. Añadir parámetro `meta_judge_reason: str \| None = None` a `_draft_best_effort_fallback`. | `backend/app/agent/orchestrator.py` | S | [be-agent] |
...
| Test file | Test name | Status |
|---|---|---|
| `tests/test_orchestrator_meta_stop_standard.py` | `test_happy_path_skips_meta_judge` — judge approve + threshold → 0 META_STOP_VERDICT events | NEW |
| `tests/test_orchestrator_meta_stop_standard.py` | `test_voc_stop_terminates_judge_confirmed_above_threshold` | NEW |
| `tests/test_orchestrator_meta_stop_standard.py` | `test_voc_stop_terminates_best_effort_below_threshold` | NEW |
| `tests/test_orchestrator_meta_stop_standard.py` | `test_voc_stop_best_effort_goes_through_fallback` — verifica `stop_reason=STOPPED_BY_BUDGET`, `answer_kind=BEST_EFFORT`, `stop_rationale` contiene `verdict.reason` | NEW |
| `tests/test_orchestrator_meta_stop_standard.py` | `test_voc_continue_below_delta_floor_treated_as_stop_best_effort` (AC del BRD AC-03) | NEW |
| `tests/test_orchestrator_meta_stop_standard.py` | `test_ac_all_answered_terminates_judge_confirmed_even_below_threshold` (AC-06) | NEW |
| `tests/test_orchestrator_meta_stop_standard.py` | `test_ac_unanswered_creates_directed_subclaims_and_re_searches` (AC-05) — verifica nuevo `SubClaim.origin=="adversarial_objection"` aparece en `state.sub_claims` | NEW |
| `tests/test_orchestrator_meta_stop_standard.py` | `test_hard_cap_wins_over_voc_continue` (AC-07) — `judge_attempts == max_judge_attempts` y VoC sigue diciendo `continue` → fallback | NEW |
| `tests/test_orchestrator_meta_stop_standard.py` | `test_meta_judge_event_carries_confidence_at_check` | NEW |
| `tests/test_orchestrator_meta_stop_standard.py` | `test_final_confidence_formula_unchanged_after_meta_judge` (AC-08) — replay del run; assert `final_confidence == min(S_effective, J)`, `expected_delta_s` no aparece en el cómputo | NEW |
...
| `tests/test_agent_lanes_fast.py` | `test_fast_lane_never_emits_meta_judge_events` (AC-12, BRD FR-26-12) | EXTEND |
...
Después de cada fase registrar en `.github/memory-bank/logs/decisions-history.md`:

- **D-XXX** (Phase A) — *"Meta-judge reusa `LLMRole` enum y env pool de roles existentes; rechazamos `META_JUDGE_PROVIDER/MODEL_<provider>` del BRD por consistencia."*
- **D-XXX** (Phase A) — *"`max_judge_attempts` sube de 3 a 5 con la introducción del meta-juez como criterio primario de stop."*
- **D-XXX** (Phase B) — *"Recomputamos `all_answered` server-side en vez de confiar en el LLM (defensa-en-profundidad)."*
- **D-XXX** (Phase D) — *"Hypothesis routing usa Jaccard sobre tokens > 3 chars con umbral 0.5 — heurística sin embeddings, alineada con la regla 'no-vector-DB en V1'."*
...
- `backend/app/llm/prompts/meta_judge_voc.py`
- `backend/app/llm/prompts/meta_judge_adversarial.py`
...
- (+ 8 test files listados en cada fase)
...
- `backend/app/llm/prompts.py` (fallback clause)
- `backend/app/config.py` (+2 settings)
- `backend/app/agent/run_state.py` (+meta_judge_calls, +SubClaim.origin, max_judge_attempts default 5)
- `backend/app/agent/orchestrator.py` (refactor `_handle_judging`, `_after_judge_meta_decision`, fallback path)
- `backend/app/agent/tasks/draft.py` (inject fallback clause)
- `backend/app/agent/lanes/deep.py` (after_cove hook)
- `backend/app/agent/react/loop.py` (after_react_observation hook + routing)
- `backend/app/stopping/signals/__init__.py` (register)
