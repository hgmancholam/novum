...
**Configuración real de modelos (interfaz agnóstica al proveedor vía litellm; V1 activa solo Anthropic Claude):**

| Rol | Modelo | Familia |
|---|---|---|
| Classifier | `anthropic/claude-haiku-4-5` | Anthropic Claude |
| Planner | `anthropic/claude-sonnet-4-6` | Anthropic Claude |
| Synthesizer | `anthropic/claude-sonnet-4-6` | Anthropic Claude |
| Judge | `anthropic/claude-sonnet-4-6` | Anthropic Claude |
| Meta-judge (BRD-26) | `anthropic/claude-sonnet-4-6` | Anthropic Claude |

En V1 todos los roles corren sobre la **misma familia** (Claude). La verificación cross-family que originalmente mitigaba R6 (DeepSeek juez vs OpenAI sintetizador) queda **diferida**: la mitigación V1 se apoya en el prompt adversarial del judge + el cap `final_confidence = min(S, J)` + el desempate del meta-judge (BRD-26). La capa de interfaz soporta Anthropic, Google Gemini, OpenAI directo y GitHub Models; reactivar cross-family es cambiar una línea en `app/llm/models.py` + exportar la API key correspondiente (`GOOGLE_API_KEY`, `OPENAI_API_KEY` o `GITHUB_TOKEN`).
...
- El judge como verificador independiente (mismo modelo que el synthesizer en V1 — misma familia Anthropic; cross-family deferred, ver hallazgos arriba).
...
| **STANDARD** | Decomp paralelo + re-decomp dinámica + CoVe vía judge + deep-fetch | **90** | Medio (~8–10 calls) | **Media (~90–120s)** | **Medio** (judge adversarial + meta-judge BRD-26 — misma familia Claude en V1, cross-family deferred) + re-decomp cierra gaps | `comparative`, `weighted`, `state_of_art`, `direct` complejo |
...
| **STANDARD** | resto | Judge `approve` con threshold + deep-fetch reactivo + best-effort fallback | Judge (`anthropic/claude-sonnet-4-6`, misma familia que el synthesizer en V1; cross-family diferido) + prompt adversarial + meta-judge BRD-26 + re-decomp dinámica + echo chamber detection + query reformulation | igual + 1–2 LLM calls extra | ~0.45x |
...
- **Judge con prompt adversarial + cap `min(S, J)` + meta-judge BRD-26** (todos los roles en `anthropic/claude-sonnet-4-6` en V1; cross-family diferido).
