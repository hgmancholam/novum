...
| **RF-20 (new)** | — | **Per-call cost & token transparency.** Every external billable call MUST emit one append-only `CostIncurred` event recording provider, kind, model, task name, tokens, units, USD cost, latency, and the source of the price. Per-run totals MUST be derivable purely from the event log (no out-of-band sidecar). The UI MUST surface the running total in real time. See [ai-services.md](../technical-phase/ai-services.md) §6 and [BRD-29](../implementation-phase/brds/BRD-29-cost-and-token-tracking.md). |
